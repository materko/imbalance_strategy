"""Koľko z obchodov by sa vyplnilo ako maker — a čo to spraví s výsledkom.
    ...
    python -m tester.fees                       # posledny backtest
    python -m tester.fees vysledok.zip
    python -m tester.fees --maker 0.02 --taker 0.05
...
def load(path: Path):
    """(stats, trades) z výsledkového zipu — rovnaký tvar ako v `tester.report`."""
    import pandas as pd
...
def classify(stats, trades, exchange: str = "binance"):
    ...
    from .compare.scan_zones import _load
