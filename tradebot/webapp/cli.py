"""Príkazový riadok nad webapp — pre Claude Code a skripty, výsledky idú do histórie.
    ...
    python -m tradebot.webapp.cli run --profile docs/profily_archiv/ibs/btcusdt_3m_binance_ny_sl_risk1.json \\
    ...
    python -m tradebot.webapp.cli show 20260905-160921-0310ba
    python -m tradebot.webapp.cli status          # beží webapp? čo je vo fronte? stav gitu
    python -m tradebot.webapp.cli pull | push     # história behov z/na GitHub
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    for item in args.set or []:
        k, v = parse_set(item)
        if k not in params:
            raise SystemExit(f"neznámy parameter {k!r} (pozri `python -m tradebot.webapp.cli params`)")
        params[k] = v
