"""IBSNet — IBS Imbalance Breakout s jadrom v C#.

Logika je prepis `tradebot/strategies/ibs` do C# (`csharp/TradeBot.Strategies/IbsNet`) nad
...
`tradebot/tests/test_ibsnet_parity.py` (bar po bare proti Python enginu).
"""
...
from .config import CONFIG_DIR, IBSNetConfig
from .hyperopt import IBSNetHyperopt
from .meta import FEATURES, INERT_INPUTS, KIND_TITLES, LAYERS, PARAM_NOTES
from .params import GROUPS, PARAMS

#: Kľúč C# enginu — `[TradeBotEngine("ibsnet", ...)]` na triede `IbsEngine`.
CSHARP_KEY = "ibsnet"

SPEC = StrategySpec(
    key="ibsnet",
    title="IBSNet Imbalance Breakout (C# jadro)",
    config_cls=IBSNetConfig,
    ...
    csharp_dir=CSHARP_DIR / "TradeBot.Strategies" / "IbsNet",
    freqtrade_class="IBSNetStrategy",
    multicharts_class="IBSNetSignal",
    multicharts_template="IBSNet_Signal.py",
    ...
    hyperopt_cls=IBSNetHyperopt,
)

__all__ = ["SPEC", "CSHARP_KEY", "IBSNetConfig", "IBSNetHyperopt", "CONFIG_DIR"]
