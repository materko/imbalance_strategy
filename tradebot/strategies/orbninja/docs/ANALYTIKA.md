# Základná analytika — ORBNinja Opening Range Breakout (C# jadro) (`orbninja`)
...
**0. Vzťah k ORB.**
ORBNinja je prepis `orb` do C#, nie nová stratégia: päť referenčných okien dalo **ten istý počet
...
([merania/PARITA_orbninja_2026-09-24.md](../../../../docs/merania/PARITA_orbninja_2026-09-24.md)).
...
ORBNinja je platforma: tá istá logika beží natívne v NinjaTraderi 8 na futures (MNQ/NQ), kde
...
Každá zmena logiky ide do `orb` aj do `csharp/…/OrbNinja` naraz a musí prejsť paritou
(`tester/tests/test_csharp_parity_orb.py`).

**4. Čo otestovať ďalej.**
- Strategy Analyzer v NinjaTraderi na MNQ s exportom signálov a
  `python -m tester.ninjatrader compare --strategy orbninja …` — overí adaptér vrátane
  `entryMode=stop` (stop-market vstup, ktorý IBSNinja nepoužíva).
- `cli run --engine freqtrade` a checkup s reálnym poplatkom — rozhodne, či edge prežije náklad.
- `cli hyperopt --strategy orbninja --param rrRatio=1.0:2.0:0.1 --param slRangePct=10:60:5`
