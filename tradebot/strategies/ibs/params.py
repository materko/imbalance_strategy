...
PARAMS: dict[str, dict[str, Any]] = {
    ...
    "tradeDirection": dict(group="🎯 Obchodovanie", title="Smer obchodov",
        tooltip="Ktorym smerom moze strategia obchodovat - Both=oba smery, Long only=len LONG "
                "(Demand zony), Short only=len SHORT (Supply zony), Indicator=smer urcuje "
                "zvoleny indikator (Indikátor smeru).",
    ),
    "indSupertrend": dict(group="🎯 Obchodovanie", title="Indikátor: Supertrend",
        tooltip="Pri Smer obchodov = Indicator: Supertrend (hore / dole) na vlastnom TF. Da sa "
                "kombinovat s ADX/DMI; co robit pri ktorej kombinacii, urcuju pravidla nizsie.",
    ),
    "stTimeframe": dict(group="🎯 Obchodovanie", title="Supertrend: TF výpočtu",
        options=["1", "3", "5", "15", "30", "45", "60", "120", "180", "240", "D"],
        tooltip="Timeframe, na ktorom sa Supertrend pocita (D = den UTC). Sklada sa z barov "
                "grafu, takze musi byt nasobkom TF grafu (na 3m grafe napr. 15, 30, 45, 60, "
                "120, 180, 240, D). Pouzije sa vzdy posledny UZAVRETY bar tohto TF.",
    ),
    "stAtrPeriod": dict(group="🎯 Obchodovanie", title="Supertrend: ATR Period",
        tooltip="Dlzka ATR Supertrendu (TradingView default 10).",
    ),
    "stSource": dict(group="🎯 Obchodovanie", title="Supertrend: Source",
        tooltip="Cena, od ktorej sa odratava/pripocitava nasobok ATR (default hl2).",
    ),
    "stMultiplier": dict(group="🎯 Obchodovanie", title="Supertrend: ATR Multiplier", step=0.1,
        tooltip="Nasobok ATR - vzdialenost pasma od ceny (default 3.0). Mensi = citlivejsi, "
                "castejsie otocenia.",
    ),
    "stChangeAtr": dict(group="🎯 Obchodovanie", title="Supertrend: Change ATR Calculation Method",
        tooltip="Zapnute = ATR ako RMA (atr()), vypnute = SMA z True Range. Default zapnute.",
    ),
    "stShowSignals": dict(group="🎯 Obchodovanie", title="Supertrend: Show Buy/Sell Signals",
        tooltip="Stitky Buy/Sell na grafe behu pri otoceni trendu. Na obchody nema vplyv.",
    ),
    "stHighlighting": dict(group="🎯 Obchodovanie", title="Supertrend: Highlighter",
        tooltip="Farebna vypln medzi ciarou Supertrendu a cenou na grafe behu. Na obchody nema vplyv.",
    ),
    "indAdx": dict(group="🎯 Obchodovanie", title="Indikátor: ADX/DMI",
        tooltip="Pri Smer obchodov = Indicator: ADX/DMI na vlastnom TF - hore (+DI > -DI), dole "
                "(-DI > +DI) alebo do strany (ADX pod prahom). Da sa kombinovat so Supertrendom.",
    ),
    "adxTimeframe": dict(group="🎯 Obchodovanie", title="ADX/DMI: TF výpočtu",
        options=["1", "3", "5", "15", "30", "45", "60", "120", "180", "240", "D"],
        tooltip="Timeframe, na ktorom sa ADX/DMI pocita (D = den UTC). Musi byt nasobkom TF grafu. "
                "Pouzije sa vzdy posledny UZAVRETY bar tohto TF.",
    ),
    "adxDiLength": dict(group="🎯 Obchodovanie", title="ADX/DMI: DI Length",
        tooltip="Dlzka +DI / -DI (TradingView default 14).",
    ),
    "adxSmoothing": dict(group="🎯 Obchodovanie", title="ADX/DMI: ADX Smoothing",
        tooltip="Vyhladenie ADX (TradingView default 14).",
    ),
    "adxThreshold": dict(group="🎯 Obchodovanie", title="ADX/DMI: prah trendu (pod = do strany)", step=1.0,
        tooltip="ADX pod touto hodnotou = trh do strany. Nad nou smer podla vacsieho z +DI / -DI. "
                "Bezne 20-25.",
    ),
    "adxShowState": dict(group="🎯 Obchodovanie", title="ADX/DMI: podfarbiť graf podľa stavu",
        tooltip="Pozadie grafu behu zelene (hore), cervene (dole), sive (do strany). Na obchody nema vplyv.",
    ),
    "ruleStUp": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend hore →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStDown": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend dole →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleAdxUp": dict(group="🎯 Obchodovanie", title="Pravidlo: ADX hore →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleAdxDown": dict(group="🎯 Obchodovanie", title="Pravidlo: ADX dole →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleAdxSide": dict(group="🎯 Obchodovanie", title="Pravidlo: ADX do strany →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStUpAdxUp": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend hore + ADX hore →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStUpAdxSide": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend hore + ADX do strany →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStUpAdxDown": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend hore + ADX dole →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStDownAdxUp": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend dole + ADX hore →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStDownAdxSide": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend dole + ADX do strany →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
    "ruleStDownAdxDown": dict(group="🎯 Obchodovanie", title="Pravidlo: Supertrend dole + ADX dole →",
        tooltip="Co robit, ked indikatory ukazuju tuto kombinaciu: Both = long aj short, Long only / Short only = len jeden smer, No trade = zona order nepolozi (SKIP).",
    ),
