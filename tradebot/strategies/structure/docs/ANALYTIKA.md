...
- 2550 obchodov spolu — na štatistiku dosť
...
- break-even -0.0087 % je pod poplatkom 0.0500 % — pri tomto poplatku je to strata, nech PnL ukazuje čokoľvek
...
- nie je lepšia než náhodný vstup za tých istých pravidiel (-0.4 sigma) — výber vstupu nepridáva nič
- najhoršia skupina 'proti trendu' vlastnosti 'S trendom, alebo proti': 1399 obchodov (54.9 %), bez nej by break-even bol o +0.0435 lepší
...
Zisková v **0 z 5** okien, obchodov spolu 2550, break-even celkom -0.0087 %. Okná sa prekrývajú, 56 obchodov započítaných dvakrát vypadlo.
...
- medián držania 54.9 barov grafu
- winrate 32.43 %, payoff 1.634
- 1.42 obchodov za deň
- šikmosť výnosov +1.108
...
Výstupy: `stop_loss` 50.2 %, `roi` 32.4 %, `trailing_stop_loss` 17.2 %, `force_exit` 0.2 %

Čo z čísel vyplýva pre tento typ: [docs/TYPY_STRATEGII.md](../../../../docs/TYPY_STRATEGII.md).

## Ktorá skupina obchodov kazí výsledok

Najhorsia skupina je 'proti trendu' vlastnosti 'S trendom, alebo proti': 1399 obchodov (54.9 %), break-even -0.0413 % oproti -0.0087 % celku. Bez nej by break-even bol 0.0348 % (+0.0435). Je to ale VACSINA obchodov, takze to nie je filter, ale nastavenie: hladaj parameter, ktory tuto vlastnost riadi.
...
|---|---|---|---|---|---|---|
| proti trendu | 1399 | 54.9 % | 29.7 | -0.0413 | 0.0348 | +0.0435 |
| s trendom | 1151 | 45.1 % | 35.8 | 0.0348 | -0.0413 | -0.0326 |

**Sviatok na burze v USA**

| skupina | obchodov | podiel | WR % | break-even % | bez nej | zmena |
|---|---|---|---|---|---|---|
| sviatok v USA (Deň M. L. Kinga) | 8 | 0.3 % | 12.5 | -0.2947 | -0.0079 | +0.0008 |
| polovičný deň (deň pred Dňom nezávislosti) | 9 | 0.4 % | 22.2 | -0.1728 | -0.0082 | +0.0005 |
| polovičný deň (deň po Vďakyvzdaní) | 11 | 0.4 % | 27.3 | -0.0584 | -0.0084 | +0.0003 |
| sviatok v USA (Deň nezávislosti) | 13 | 0.5 % | 23.1 | -0.0131 | -0.0086 | +0.0001 |
| deň pred sviatkom | 50 | 2.0 % | 30.0 | -0.0119 | -0.0086 | +0.0001 |
| bežný deň | 2365 | 92.7 % | 32.4 | -0.0103 | 0.0129 | +0.0216 |
| deň po sviatku | 63 | 2.5 % | 44.4 | 0.1469 | -0.0126 | -0.0039 |

**Kde v rozsahu, v smere obchodu**

| skupina | obchodov | podiel | WR % | break-even % | bez nej | zmena |
|---|---|---|---|---|---|---|
| do 0.4979 | 638 | 25.0 % | 28.1 | -0.0583 | 0.0105 | +0.0192 |
| 0.4979 – 0.6784 | 638 | 25.0 % | 30.7 | -0.0218 | -0.0040 | +0.0047 |
| 0.6784 – 0.8343 | 637 | 25.0 % | 33.8 | 0.0079 | -0.0140 | -0.0053 |
| nad 0.8343 | 637 | 25.0 % | 37.2 | 0.0515 | -0.0256 | -0.0169 |
...
|---|---|---|---|---|
| anytime (vstupy kedykoľvek v okne) | -0.0048 % | 0.0003 % ± 0.0133 | -0.38 | 35.8 |
| session (vstupy v tých istých hodinách, v akých obchoduje stratégia) | -0.0048 % | 0.0003 % ± 0.0133 | -0.38 | 35.8 |

Neodlisitelne od nahody (-0.4 sigma, percentil 35.8). Vyber vstupu k vysledku nepridava nic, co by sa nedalo dostat aj hodom mincou.
...
|---|---|---|---|---|---|
| 2021-10 - 2022-12 | 645 | 43.7 | 31.8 | -0.0202 | 31 |
| 2022-12 - 2024-03 | 594 | 40.3 | 32.3 | -0.0123 | 45 |
| 2024-03 - 2025-06 | 739 | 50.1 | 33.8 | 0.0055 | 75 |
| 2025-06 - 2026-09 | 572 | 38.8 | 31.5 | -0.0073 | 53 |

Hranice pre úsek veľkosti posledného obdobia: -0.0439 až +0.0289 % (medián -0.0092).

**DRZI** — posledné obdobie (2025-06 - 2026-09) je na 53. percentile, teda v medziach -0.0439 až +0.0289 %, ktoré tá istá stratégia vyrobí sama od seba. Úpadok v dátach vidieť nie je.

## Interval okolo výsledku a čo to robí s účtom

Bootstrap po blokoch 10 obchodov, 10 000 opakovaní.

- break-even: namerané **-0.0087 %**, medián -0.0085 %, 90 % interval -0.0260–0.0083 %
- P(edge > poplatok) = **0.0 %**
...
winrate **33,3 %**; nameraných je **32,4 %** na 2 550 obchodoch (56 obchodov z prekryvu
okien 2023-10→2024-10 a 2024-09→2025-09 sa počíta raz). Výber vstupu podľa štruktúry
teda dáva presne to, čo dá geometria RR 2 sama od seba, a test proti náhode to potvrdzuje
nezávisle (−0,38 sigma, 35,8. percentil; náhoda losovaná z tých istých piatich okien).
Break-even −0,0087 % proti poplatku 0,0500 % nie je „skoro tam": je to nula.
...
−0,38 sigma proti náhodným vstupom a P(edge > poplatok) = 0,0 %. Test úpadku hlási `DRZI`,
...
- **Žiadny filter režimu.** Štruktúrny prielom v rozsahovom trhu je falošný takmer vždy, a
  práve tie tvoria väčšinu z tých ~500 obchodov ročne. Toto je najväčšia diera a teraz to
  vidieť priamo: **obchody proti sklonu posledných 50 barov** sú 55 % všetkých a majú
  break-even −0,0413 %, kým obchody s trendom +0,0348 % — bez tých proti trendu by celok
  bol nad nulou (nie nad poplatkom, ale nad nulou). K tomu vstupy v spodnej štvrtine
  rozsahu (v smere obchodu) −0,0583 % oproti +0,0515 % v hornej. Je to väčšina obchodov,
  takže nie filter, ale chýbajúci parameter: „obchoduj len v smere sklonu" stratégia dnes
  nemá a `FEATURE_PARAMS` naň nemá čo ukázať.
...
**5. Je to použiteľné?** **Nie.** Na BTC 5m je to overené na 2 550 obchodoch, piatich oknách,
...
**6. Čo by som pridal.** Dve veci, obe plynú priamo z čísel. Prvá je **filter smeru
voči sklonu** (`trendFilter`: obchod len v smere zmeny ceny za posledných N barov, default
vypnutý): obchody proti sklonu sú 55 % vzorky s break-even −0,041 %, s trendom +0,035 %,
a je to jediná vopred známa vlastnosť, ktorá oddeľuje dve populácie s opačným znamienkom.
Druhá je **prah prielomu v ATR**. Dnes je BOS aj CHoCH binárne — zavretie o tick za úrovňou
váži rovnako ako zavretie o 2 ATR za ňou. Winrate 32,4 % proti potrebným 33,3 % znamená, že
setup nie je zlý, len sa **v tej istej vrecke miešajú prielomy s prepichnutiami**. Jeden
parameter (`minBreakSize` v `atr`, default 0 = dnešné správanie) by tie dve veci oddelil
a dal by sa zmerať sweepom rovnako ako `minSwingSize`. Až keby ani jedno nepomohlo na
žiadnom zo 16 trhov, je myšlienka „štruktúra ako signál" vyčerpaná.
