...
from tradebot.webapp import chart as chart_mod
from tradebot.webapp import runner as runner_mod
...
def mc_data(tmp_path: Path, monkeypatch) -> Path:
    ...
    path = mc_dir / f"{INST.data_stem}-1m.feather"
...
def test_beh_na_burze_multicharts_ide_cez_emulator_a_ulozi_sa(mc_data, tmp_path: Path, monkeypatch):
    ...
    from tradebot.webapp.store import RunStore
    ...
    from tradebot.webapp.store import make_run_id
