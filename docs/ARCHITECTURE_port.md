...
Referenčný zdroj pravdy: [`tradebot/strategies/ibs/docs/sources/imbalance_strategy_FULL.pine`](../tradebot/strategies/ibs/docs/sources/imbalance_strategy_FULL.pine) +
...
2. ✅ **hotové** — `core/clock.py` + `core/zones.py` + `core/drawing.py`: session okná, detekcia SD
   ...
   Overené na reálnych dátach cez `python -m tester.compare.scan_zones`.
3. ✅ **hotové** — `core/history.py`, `core/ta/{imbalance,patterns}.py`, `core/risk.py`,
   ...
   a Engulfing modelu. Overené cez `python -m tester.compare.scan_trades`.
