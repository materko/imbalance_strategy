...
Tieto profily boli do 2026-09-05 v `ibs/configs/` a webapp ich ponúkala v zozname
východiskových profilov. Boli to medzikroky vývoja a experimenty; každú kombináciu si
tester nastaví vo formulári alebo cez `--set`, a história behov ju drží. V `ibs/configs/`
...
```bash
python -m ibs.webapp.cli run --profile docs/profily_archiv/btcusdt_3m_binance_ny_sl_risk1.json --timerange 20250904-20260904 --note "..."
IBS_PROFILE=docs/profily_archiv/btcusdt_3m_binance_hyper.json ./platforms/freqtrade/scripts/hyperopt.sh 20250904-20260904 200
```

Staršie dokumenty v `docs/` sa na ne odkazujú pôvodnými názvami — sú to záznamy meraní,
nemenili sa.

## Premenované referenčné profily (ostávajú v `ibs/configs/`)
