# Šablóna MultiCharts študie pre IBSEntryZone (IBS s jadrom v C#; treba pythonnet) — skopíruj obsah do PowerLanguage .NET Editora.
#
#   File → New → Signal, jazyk Python, názov študie IBSEntryZone (= trieda nižšie), potom sem
...
# z tradebot/strategies/ibsentry/configs), alebo natvrdo nižšie cez PROFILE (názov alebo cesta k JSON).
...
class IBSEntryZone:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na IBSEntryZoneSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.ibsentry.multicharts import IBSEntryZoneSignal

            s = IBSEntryZoneSignal()
