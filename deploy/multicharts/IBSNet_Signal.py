# Šablóna MultiCharts študie pre IBSNet (IBS s jadrom v C#; treba pythonnet) — skopíruj obsah do PowerLanguage .NET Editora.
#
#   File → New → Signal, jazyk Python, názov študie IBSNet (= trieda nižšie), potom sem
...
# z tradebot/strategies/ibsnet/configs), alebo natvrdo nižšie cez PROFILE (názov alebo cesta k JSON).
...
class IBSNet:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na IBSNetSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.ibsnet.multicharts import IBSNetSignal
