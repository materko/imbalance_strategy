# Šablóna MultiCharts študie pre divergenčnú stratégiu — skopíruj obsah do
# PowerLanguage .NET Editora (File → New → Signal, jazyk Python, názov Divergence = trieda nižšie).
#
...
# len delegujú na balík. Vyššie TF si engine skladá z barov grafu, na grafe stačí Data1.
#
# Profil sa prepína premennou prostredia TRADEBOT_PROFILE (predvolene "binance_btcusdt_15m"
# z tradebot/strategies/divergence/configs/), alebo natvrdo nižšie cez PROFILE.
...
class Divergence:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na DivergenceSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.divergence.multicharts import DivergenceSignal
