# Šablóna MultiCharts študie pre Gap Fill — skopíruj obsah do
# PowerLanguage .NET Editora (File → New → Signal, jazyk Python, názov Gap = trieda nižšie).
#
...
#
# Profil sa prepína premennou prostredia TRADEBOT_PROFILE (predvolene "nas100_dukascopy_5m"
# z tradebot/strategies/gap/configs), alebo natvrdo nižšie cez PROFILE.
...
class Gap:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na GapSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.demo_breakout.multicharts import GapSignal
