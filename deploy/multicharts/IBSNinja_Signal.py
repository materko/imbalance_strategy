# Šablóna MultiCharts študie pre IBSNinja (IBS s jadrom v C#; treba pythonnet) — skopíruj obsah do PowerLanguage .NET Editora.
#
#   File → New → Signal, jazyk Python, názov študie IBSNinja (= trieda nižšie), potom sem
...
# z tradebot/strategies/ibsninja/configs), alebo natvrdo nižšie cez PROFILE (názov alebo cesta k JSON).
...
class IBSNinja:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na IBSNinjaSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.ibsninja.multicharts import IBSNinjaSignal
