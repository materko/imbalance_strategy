# Šablóna MultiCharts študie pre ORBNet (ORB s jadrom v C#; treba pythonnet) — skopíruj obsah do
# PowerLanguage .NET Editora (File → New → Signal, jazyk Python, názov ORBNet = trieda nižšie).
#
...
# z tradebot/strategies/orbnet/configs), alebo natvrdo nižšie cez PROFILE.
...
class ORBNet:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na ORBNetSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.orbnet.multicharts import ORBNetSignal
