# Šablóna MultiCharts študie pre SD Zones (supply-demand z bazy a impulzu) — skopíruj obsah do
...
#
# Profil sa prepína premennou prostredia TRADEBOT_PROFILE (predvolene "xau_dukascopy_15m"
# z tradebot/strategies/sdzone/configs), alebo natvrdo nižšie cez PROFILE.
...
class SDZone:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na SDZoneSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.sdzone.multicharts import SDZoneSignal
