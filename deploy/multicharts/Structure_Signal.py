# Šablóna MultiCharts študie pre Market Structure BOS / CHoCH — skopíruj obsah do
# PowerLanguage .NET Editora (File → New → Signal, jazyk Python, názov Structure = trieda nižšie).
#
...
# z tradebot/strategies/structure/configs/), alebo natvrdo nižšie cez PROFILE.
...
class Structure:
    ...
    HTF_CSV = None   # štruktúra nemá informatívny TF; pole je tu len pre jednotný tvar šablóny
    NO_DRAW = False
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na StructureSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.structure.multicharts import StructureSignal
