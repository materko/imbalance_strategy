# Šablóna MultiCharts študie pre IBSZones (IBS s jadrom v C#; treba pythonnet) — skopíruj obsah do PowerLanguage .NET Editora.
#
#   File → New → Signal, jazyk Python, názov študie IBSZones (= trieda nižšie), potom sem
...
# z tradebot/strategies/ibszones/configs), alebo natvrdo nižšie cez PROFILE (názov alebo cesta k JSON).
...
class IBSZones:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na IBSZonesSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.ibszones.multicharts import IBSZonesSignal
