# Šablóna MultiCharts študie pre IBSFvg (IBS s jadrom v C#; treba pythonnet) — skopíruj obsah do PowerLanguage .NET Editora.
#
#   File → New → Signal, jazyk Python, názov študie IBSFvg (= trieda nižšie), potom sem
...
# z tradebot/strategies/ibsfvg/configs), alebo natvrdo nižšie cez PROFILE (názov alebo cesta k JSON).
...
class IBSFvg:
    ...
    # trieda bez rodiča ako v šablóne bety; všetko deleguje na IBSFvgSignal.
    def _sig(self):
        s = getattr(self, "_tb", None)
        if s is None:
            from tradebot.strategies.ibsfvg.multicharts import IBSFvgSignal
